...
public class IPServiceImpl implements IPService {
	...
	public IPResponse search(String ip) {
		try{
			...
			parser.setPath(IPServiceImpl.class.getResource(this.source).getFile());
			parser.seek(ip);
			return new IPResponseImpl(null, parser.getCountry() + " " + parser.getLocal());
		}catch(Exception e){
			return new IPResponseImpl("未知", null);
		}
	}

	private class IPResponseImpl implements IPResponse{

		private String error;
		
		private String address;
		
		public IPResponseImpl(String error, String address) {
			super();
			this.error = error;
